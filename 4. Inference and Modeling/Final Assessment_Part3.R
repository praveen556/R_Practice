...
#and then calculates whether the confidence interval covers the actual value of the spread  ????=???0.038 :
brexit_hit <- brexit_polls %>%
  mutate(p_hat = (spread + 1)/2,
         se_spread = 2*sqrt(p_hat*(1-p_hat)/samplesize),
         spread_lower = spread - qnorm(.975)*se_spread,
...
(two_by_two$Online[two_by_two$hit == 'TRUE'])/(two_by_two$Telephone[two_by_two$hit == 'TRUE'])
...
#Add a horizontal line indicating the final value of  ????=???.038 .
brexit_polls %>%
